...
func newCustomFixture(t *testing.T, mutateCfg func(*auth.TestServerConfig), sshOpts ...ServerOption) *sshTestFixture {
	...
	client, err := tracessh.Dial(context.Background(), "tcp", sshSrv.Addr(), cltConfig)
	require.NoError(t, err)
...
func TestLockInForce(t *testing.T) {
	...
	newClient, err := tracessh.Dial(context.Background(), "tcp", f.ssh.srvAddress, f.ssh.cltConfig)
	require.NoError(t, err)
	t.Cleanup(func() {
	...
	newClient2, err := tracessh.Dial(context.Background(), "tcp", f.ssh.srvAddress, f.ssh.cltConfig)
	require.NoError(t, err)
...
func TestMaxSessions(t *testing.T) {
	...
	_, err = f.ssh.clt.NewSession(context.Background())
	require.Error(t, err)
	require.Contains(t, err.Error(), "too many session channels")

	// verify that max sessions does not affect max connections.
	for i := int64(0); i <= maxSessions; i++ {
		clt, err := tracessh.Dial(context.Background(), "tcp", f.ssh.srv.Addr(), f.ssh.cltConfig)
		require.NoError(t, err)
...
func TestX11Forward(t *testing.T) {
	...
	client2, err := tracessh.Dial(context.Background(), "tcp", f.ssh.srv.Addr(), f.ssh.cltConfig)
	require.NoError(t, err)
...
func TestLimiter(t *testing.T) {
	...
	clt0, err := tracessh.Dial(context.Background(), "tcp", srv.Addr(), config)
	require.NoError(t, err)
	require.NotNil(t, clt0)

	se0, err := clt0.NewSession(context.Background())
	require.NoError(t, err)
	require.NoError(t, se0.Shell())

	// current connections = 2
	clt, err := tracessh.Dial(context.Background(), "tcp", srv.Addr(), config)
	require.NotNil(t, clt)
	require.NoError(t, err)
	se, err := clt.NewSession(context.Background())
	require.NoError(t, err)
	...
	_, err = tracessh.Dial(context.Background(), "tcp", srv.Addr(), config)
	require.Error(t, err)
	...
	require.NotNil(t, clt)
	require.NoError(t, err)
	_, err = clt.NewSession(context.Background())
	require.Error(t, err)
...
func TestIgnorePuTTYSimpleChannel(t *testing.T) {
	...
	client, err := tracessh.Dial(context.Background(), "tcp", proxy.Addr(), sshConfig)
