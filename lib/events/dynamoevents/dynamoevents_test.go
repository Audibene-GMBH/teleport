...
func (tt *dynamoContext) Close() error {
	if tt.log != nil {
		return tt.log.deleteTable(context.Background(), tt.log.Tablename, true)
	}
...
func TestSizeBreak(t *testing.T) {
	...
	for _, event := range events {
		require.Equal(t, event.GetTime().Before(lastTime), true)
...
func TestLargeTableRetrieve(t *testing.T) {
	...
	require.Equal(t, len(history), eventCount)
}
